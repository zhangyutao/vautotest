...
::a raw xml from Server Manager Configuration
...
ServerManagerCmd -query %tempSMCQuery%>%tempSMCQueryRes% 2>&1
if %ERRORLEVEL% GTR 0 (goto erorhandle) else ( goto continue )
:erorhandle
echo ^<ServerManagerConfigurationQuery^>>>%filePath%
type %tempSMCQueryRes%>>%filePath%
echo ^<^/ServerManagerConfigurationQuery^>>>%filePath%
goto nextsteps
:continue
type %tempSMCQuery%>>%filePath%
del %tempSMCQuery%
:nextsteps
del %tempSMCQueryRes%
echo ^<^/servermanagerconfiguration^>>>%filePath%
